...
public final class DBHelper {
	...
	/**
	 * 1. Añadir un usuaario
	 */
	public static void insertaUsuario(BasicDBObject usuario){
		DBHelper.dataBase.insert(Constantes.USUARIOS, usuario);
	}
	 ...
	 * 2. Añadir un cine
	 */
	public static void insertaCine(BasicDBObject cine){
		
		DBHelper.dataBase.insert(Constantes.CINES, cine);
	...
	/**
	 * 3. Conocer qué usuarios han visto una película o episodio
	 */
	public static void queUsuarioHaVisto(String coleccion, String titulo, BasicDBObject episodio){
		
		// El valor de coleccion se reduce a Constantes.PELICULAS o Constantes.EPISODIOS, que son las que el usuario puede haber visto
		ArrayList<String> usuarios = new ArrayList<String>();
		if(coleccion.equals(Constantes.PELICULAS)){
			DBCursor cursor = DBHelper.dataBase.getAll(Constantes.USUARIOS, new BasicDBObject("historial.pelicula", titulo));
			try {
				String nombre;
				while(cursor.hasNext()) {
					nombre = (String) cursor.next().get("nombre");
			...
			Iterator<String> it1 = usuarios.iterator();
			while(it1.hasNext()){
			...
			DBCursor cursor = DBHelper.dataBase.getAll(Constantes.USUARIOS, new BasicDBObject("episodios", idEpisodio));
			try {
				String nombre;
				while(cursor.hasNext()) {
					nombre = (String) cursor.next().get("nombre");
			...
			Iterator<String> it2 = usuarios.iterator();
			while(it2.hasNext()){
				System.out.println(it2.next());
			}
		}
		else{
			System.err.println("La colección no es ni película ni episodio");
		}
		
	}
	
	/**
	 * 4. Consultar las películas y episodios vistos por un usuario
	 */
	public static void vistosPorUsuario(String nombre){
		
		// Películas vistas por un usuario
		BasicDBObject usuario = DBHelper.dataBase.get(Constantes.USUARIOS, new BasicDBObject("nombre", nombre));
		
		ArrayList<String> peliculasVistas = new ArrayList<String>();
		ArrayList<BasicDBObject> listaPeliculas = (ArrayList<BasicDBObject>) DBHelper.dataBase.getCampo("historial", Constantes.USUARIOS, usuario);
		
		Iterator<BasicDBObject> it = listaPeliculas.iterator();
		while(it.hasNext()){
			BasicDBObject act = it.next();
			if(!peliculasVistas.contains(act.get("pelicula")));
			peliculasVistas.add((String) act.get("pelicula"));
		}
		
		ArrayList<String> episodiosVistos = new ArrayList<String>();
		ArrayList<String> listaEpisodios = (ArrayList<String>) DBHelper.dataBase.getCampo("episodios", Constantes.USUARIOS, usuario);
		
		Iterator<String> itEpisodios = listaEpisodios.iterator();
		String idEpisodio;
		String nombreEpisodio;
		while(itEpisodios.hasNext()){
			idEpisodio = itEpisodios.next();
			BasicDBObject ep = DBHelper.dataBase.getById(Constantes.EPISODIOS, new ObjectId(idEpisodio));
			nombreEpisodio = (String) DBHelper.dataBase.getCampo("titulo", Constantes.EPISODIOS, ep);
			if(!episodiosVistos.contains(nombreEpisodio)){
				episodiosVistos.add(nombreEpisodio);
			}
		}
		
		System.out.println("Películas vistas:");
		Iterator<String> i = peliculasVistas.iterator();
		while(i.hasNext()){
			System.out.println(i.next());
		}
		System.out.println("");
		
		System.out.println("Episodios vistos:");
		i = episodiosVistos.iterator();
		while(i.hasNext()){
			System.out.println(i.next());
		}
		System.out.println("");
	...
	/**
	 * 5. Ver las valoraciones realizadas por un usuario
	 */
	public static void valoracionesDeUnUsuario(String nombre){
		
		DBCursor valoraciones = DBHelper.dataBase.getAll(Constantes.VALORACIONES, new BasicDBObject("usuario", nombre));
		try {
			BasicDBObject valoracion;
			while(valoraciones.hasNext()) {
				valoracion = (BasicDBObject) valoraciones.next();
				System.out.println();
				System.out.println(valoracion.get("titulo"));
				System.out.println(valoracion.get("puntuacion"));
				System.out.println(valoracion.get("mensaje"));
				System.out.println(valoracion.get("usuario"));
			}
		} finally {
			valoraciones.close();
		}
	...
	/**
	 * 6. Ver aquellas valoraciones muy negativas (puntuación inferior a 4) de un usuario
	 */
	public static void valoracionesNegativasUsuario(String nombreUsuario){
		DBCursor valoraciones = DBHelper.dataBase.getAll(Constantes.VALORACIONES, new BasicDBObject("usuario", nombreUsuario));
		try {
			int puntuacion;
			BasicDBObject valoracion;
			while(valoraciones.hasNext()) {
				valoracion = (BasicDBObject) valoraciones.next();
				puntuacion = (int) DBHelper.dataBase.getCampo("puntuacion", Constantes.VALORACIONES, valoracion);
				if(puntuacion < 4){
					System.out.println();
					System.out.println(valoracion.get("titulo"));
					System.out.println(valoracion.get("puntuacion"));
	...
	/**
	 * 7. Ver aquellas valoraciones muy negativas (puntuación inferior a 4) de una película 
	 * @param nombrePelicula
	 */
	public static void valoracionesNegativasPelicula(String nombrePelicula){
		BasicDBObject pelicula = DBHelper.dataBase.get(Constantes.PELICULAS, new BasicDBObject("titulo", nombrePelicula));
		ArrayList<String> valoraciones = (ArrayList<String>) pelicula.get("valoraciones");
		Iterator<String> it = valoraciones.iterator();
		ArrayList<String> negativos = new ArrayList<String>();
		while(it.hasNext()){
			BasicDBObject valoracion = DBHelper.dataBase.getById(Constantes.VALORACIONES, new ObjectId(it.next()));
			int puntuacion = (int) valoracion.get("puntuacion");
			if(puntuacion < 4){
				negativos.add(valoracion.get("titulo") + " " + valoracion.get("puntuacion") + " " + valoracion.get("mensaje") + " " + valoracion.get("usuario"));
			}
		}
		Iterator<String> n = negativos.iterator();
		while(n.hasNext()){
	...
	/**
	 * 8. Consultar la cartelera de un cine dado su nombre.
	 * @param nombreCine
	 */
	public static void carteleraCine(String nombreCine){
		BasicDBObject cine = DBHelper.dataBase.get(Constantes.CINES, new BasicDBObject("nombre", nombreCine));
		ArrayList<BasicDBObject> salas = (ArrayList<BasicDBObject>) cine.get("salas");
		Iterator<BasicDBObject> it = salas.iterator();
		ArrayList<String> peliculas = new ArrayList<String>();
		while(it.hasNext()){
			BasicDBObject peli = it.next();
			if(!peliculas.contains((String) peli.get("pelicula"))){
				peliculas.add((String) peli.get("pelicula"));
			}
		}
		Iterator<String> p = peliculas.iterator();
		while(p.hasNext()){
	...
	/**
	 * 9. Consultar las películas (solo su título) que han sido vistas por algún usuario en un cine concreto. 
	 * @param nombreUsuario
	 */
	public static void peliculasVistasPorUsuario(String nombreCine){
		DBCursor usuarios = DBHelper.dataBase.getAll(Constantes.USUARIOS, new BasicDBObject("historial.cine", nombreCine));
		ArrayList<String> pelis = new ArrayList<String>();
		while(usuarios.hasNext()){
			BasicDBObject usuario = (BasicDBObject) usuarios.next();
			ArrayList<BasicDBObject> historial = (ArrayList<BasicDBObject>) usuario.get("historial");
			Iterator<BasicDBObject> itObj = historial.iterator();
			while(itObj.hasNext()){
				BasicDBObject o = itObj.next();
				if(o.get("cine").equals(nombreCine)){
					if(!pelis.contains(o.get("pelicula"))){
						pelis.add((String) o.get("pelicula"));
					}
				}
			}
			Iterator<String> p = pelis.iterator();
			while(p.hasNext()){
				System.out.println(p.next());
			}
		}
	...
	/**
	 * 10.Obtener los cines en los que un usuario (dado su nombre) ha visto películas
	 * @param nombreUsuario
	 */
	public static void cineVistoUsuario(String nombreUsuario){
		BasicDBObject usuario = DBHelper.dataBase.get(Constantes.USUARIOS, new BasicDBObject("nombre", nombreUsuario));
		ArrayList<BasicDBObject> historial = (ArrayList<BasicDBObject>) usuario.get("historial");
		Iterator<BasicDBObject> it = historial.iterator();
		ArrayList<String> cines = new ArrayList<String>(); 
		while(it.hasNext()){
			BasicDBObject registro = it.next();
			if(!cines.contains((String) registro.get("cine"))){
				cines.add((String) registro.get("cine"));
			}
		}
		Iterator<String> cine = cines.iterator();
		while(cine.hasNext()){
	...
	/**
	 * 11.Obtener los usuarios que han visto alguna película en un cine concreto
	 * @param nombreCine
	 */
	public static void usuariosVistoPeliCineConcreto(String nombreCine){
		DBCursor cursor = DBHelper.dataBase.getAll(Constantes.USUARIOS, new BasicDBObject("historial.cine", nombreCine));
		...
		Iterator<String> nombres = listaUsuarios.iterator();
